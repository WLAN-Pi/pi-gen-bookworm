...
# longer apt-install the kernel in 00-packages (that would pull 7.1).
mkdir -p "${ROOTFS_DIR}/boot/firmware/overlays"
install -m 644 files/max3421-hcd.dtbo "${ROOTFS_DIR}/boot/firmware/overlays/"

# Pin 6.12.107-v8-wlanpi+-20260831 (wlanpi-kernel #31 + #37) from GitHub.
# Encoded URL: '+' in the tag/filename must be %2B.
# Hold the package so later stages cannot apt-upgrade it to 7.1.x from packagecloud.
KERNEL_DEB_URL="https://github.com/WLAN-Pi/wlanpi-kernel/releases/download/6.12.107-v8-wlanpi%2B-20260831/wlanpi-kernel-bookworm-v8_6.12.107-v8-wlanpi%2B-20260831_arm64.deb"
